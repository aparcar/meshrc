...
class PromNetJson():
    def __init__(self):
        ...
        self.properties_active = [
            ...
            [ "load", "node_load15"], 
            [ "network", "bmx7_tunIn", "tunIn", True],
            [ "memory", "100* (node_memory_MemFree / node_memory_MemTotal)" ],
            [ "clients", "count(wifi_station_signal{ifname=~'wlan.*-ap.*'}) by (shortId)" ]
        ]
    ...
    def api_call_propertie(self, propertie, query label=None, multi=False):
